---
name: cloud-native-microservices-patterns
description: "Cloud-native service design: 12-factor compliance, liveness, readiness and startup probes, graceful shutdown with SIGTERM handling and connection draining, structured logging with correlation IDs, and idempotency. Use when pods are killed during a deploy and drop in-flight requests, when Kubernetes restarts a container that is merely slow to start, or when refactoring a service to run correctly on Kubernetes."
level: senior
tags: [cloud-native, microservices, 12-factor, kubernetes, architecture, platform-engineering]
